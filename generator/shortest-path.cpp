...
int read_E(string e_command,vector<vector<int>>* adj_list)
{
    ...
    while(regex_search(beg,end,e_result,regex("<[0-9]+,[0-9]+>")))
    {
        ...
        int num1 = stoi(e_str.substr(left+1,comma-left-1));
        int num2 = stoi(e_str.substr(comma+1,right-comma-1));
        if(num1 > _V-1 or num2 > _V-1)
        {
            cerr<<"Error: Over V limit."<<endl;
            return 0;
        }
        if(num1 == num2 )
        {
...
int print_path(int start, int endin, struct Vertex vertices[], stack<int>* path)
{
    ...
    {
        cerr<<"Error: No path from "<<start<<" to "<<endin<<" exists."<<endl;
        return 0;
    }
    else
    {
...
int main(int argc, const char * argv[])
{
    ...
    while (!std::cin.eof())
    {
        ...
        else
        {
            ...
            {
                cerr<<"Error: Wrong input. a2 command = "<<command<<endl;
            }
        }
    }
    return 0;
