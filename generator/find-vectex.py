...
#log: changed the V and E output
#log: using sys.stdout to print
#log: using if __name__ == '__main__': main()
'''Things to be done
You should edit your Python script from Assignment 1 so that 
its output format for the specication of the graph matches the input format 
for your C++ program from Assignment 2. 
        V 15
        E {<2,6>,<2,8>,<2,5>,<6,5>,<5,8>,<6,10>,<10,8>}
Also, the only output that your Python script should produce to stdout is in response to “g", 
for which it outputs the specication of the graph (i.e., V and E). Error output should go to stderr.
'''
...
def remove(command):
    ...
    if flag:
        ...
        print("Error: 'c' or 'r' specified for a street that does not exist. command = ",command,file=sys.stderr)
...
def generate(command):
    ...
    for edge in selected_result:
        ...
        if selected_result.index(edge) == (len(selected_result)-1):
            sys.stdout.write("<%d,%d>"%(vertex_index.index(temp_edge[0]),vertex_index.index(temp_edge[1])))
        else:
            sys.stdout.write("<%d,%d>,"%(vertex_index.index(temp_edge[0]),vertex_index.index(temp_edge[1])))
    sys.stdout.write("}\n")
    sys.stdout.flush()
    ...
    while True:
        try:
            ...
            if re.match(re_add,command):
                ...
                print('Error: Wrong input. command = ',command,file=sys.stderr)
        except EOFError:
            sys.exit(0)
